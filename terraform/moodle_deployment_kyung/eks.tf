...
locals {
  ...
  iam_role_policy_prefix = arnawsiam${data.aws_caller_identity.current.account_id}policy
}

module eks {
  # 모듈 사용
  source = terraform-aws-moduleseksaws
  ...
  eks_managed_node_groups = {
    (${var.cluster_name}-node-group) = {
      ...
      # 생성되는 인스턴스에 tag추가
      tags = {
        k8s.iocluster-autoscalerenabled  true
        k8s.iocluster-autoscaler${var.cluster_name}  true
      }
    }
  }
}
